...
async function migrateFamilies() {
    ...
    for (const doc of familiesSnap.docs) {
        try {
            ...
            stats.errors.push(`Family ${doc.id}: ${error.message}`);
        }
    }
 ...
 * Migrate Expenses
 */
async function migrateExpenses(familyId) {
    ...
    for (const doc of snap.docs) {
        try {
            ...
            stats.errors.push(`Expense ${doc.id}: ${error.message}`);
        }
    }
}
/**
 * Migrate Tasks
 */
async function migrateTasks(familyId) {
    const snap = await firestore
        .collection('families')
        .doc(familyId)
        .collection('tasks')
    ...
    for (const doc of snap.docs) {
        try {
            ...
            stats.errors.push(`Task ${doc.id}: ${error.message}`);
        }
    }
}
/**
 ...
 */
async function migrateCalendarEvents(familyId) {
    const snap = await firestore
        .collection('families')
        .doc(familyId)
 ...
 */
async function migrateMessages(familyId) {
    const snap = await firestore
        .collection('families')
        .doc(familyId)
        .collection('messages')
    ...
    for (const doc of snap.docs) {
        try {
            ...
            stats.errors.push(`Message ${doc.id}: ${error.message}`);
        }
    }
}
/**
 ...
 */
async function migrateSwapRequests(familyId) {
    const snap = await firestore
        .collection('families')
        .doc(familyId)
    ...
    for (const doc of snap.docs) {
        try {
            ...
            stats.errors.push(`SwapRequest ${doc.id}: ${error.message}`);
        }
    }
}
/**
 * Migrate Documents
 */
async function migrateDocuments(familyId) {
    const snap = await firestore
        .collection('families')
        .doc(familyId)
 ...
 */
async function migrateCustodySchedule(familyId) {
    const snap = await firestore
        .collection('families')
        .doc(familyId)
        .collection('settings')
    ...
    try {
        ...
        stats.errors.push(`CustodySchedule ${familyId}: ${error.message}`);
    }
}
/**
 * Migrate Finance Settings
 */
async function migrateFinanceSettings(familyId) {
    const snap = await firestore
        .collection('families')
        .doc(familyId)
        .collection('settings')
    ...
    try {
        ...
        stats.errors.push(`FinanceSettings ${familyId}: ${error.message}`);
    }
}
/**
 * Update user's active family
 */
async function updateActiveFamily() {
...
main();
//# sourceMappingURL=migrate-from-firestore.js.map